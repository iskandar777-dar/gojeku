...
if ($register == false)
	...
	if ($verif == false)
		...
		{
		echo "Ready to Claim Voucher Food1\n";
		$data = '{"promo_code":"GOFOODBOBA19"}';
		$claim = claim($verif, $data);
		if ($claim == false){
			echo "Failed to Claim Voucher Food1, Coba redeem voucher lain\n";
			echo "Sleep 10 seconds";
			sleep(10);
			echo "Ready to Claim Food2\n";
			$data = '{"promo_code":"GOFOODBOBA10"}';
			$claim2 = claim($verif, $data);
			if($claim2 == false) {
				echo "Failed to claim voucher Food2";
				echo "Sleep 10 seconds";
				sleep(10);
				echo "Ready to Claim Food3\n";
				$data = '{"promo_code":"GOFOOD021120A"}';
				$claim3 = claim($verif, $data);
				if($claim3 == false) {
					echo "Failed to claim voucher Food3";
				}else{
					echo $claim2 . "\n";
				}
			}else{
